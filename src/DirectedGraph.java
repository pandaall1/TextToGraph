...
class DirectedGraph {
    ...
    // 寻找从源节点到目标节点的最短路径长度
    public Map<String, String> findShortestPath(String source, String destination) {
        Map<String, Integer> distanceMap = new HashMap<>(); // 存储节点到源节点的距离
        Map<String, String> parentMap = new HashMap<>(); // 存储节点的父节点，用于回溯路径
        Queue<String> queue = new LinkedList<>(); // 用于广度优先搜索

        distanceMap.put(source, 0); // 源节点到自身的距离为0
        queue.offer(source); // 将源节点加入队列

        while (!queue.isEmpty()) {
            String current = queue.poll(); // 出队列
            if (current.equals(destination)) { // 到达目标节点
                return parentMap; // 返回最短路径长度
            }
            ...
            if (currentNode != null) {
                for (Node neighbor : currentNode.getNeighbors()) { // 遍历当前节点的邻居节点
                    ...
                    if (!distanceMap.containsKey(neighborName)) { // 如果邻居节点未被访问过
                        distanceMap.put(neighborName, distanceMap.get(current) + 1); // 更新距离
                        parentMap.put(neighborName, current); // 记录父节点
        ...
        return Collections.emptyMap(); // No path found
    }
